...
function isCashier() {
  return state.currentUser?.role === 'CASHIER';
}
...
export async function loadEmployees() {
  ...
  if (!tbody) return;
  tbody.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-slate-400"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data akun...</td></tr>`;
  ...
  try {
    ...
    if (result.success && result.data && result.data.length > 0) {
      ...
      const selfOnly = !!result.self_only || isCashier();

      tbody.innerHTML = result.data
        .map((u) => {
          const isSelf = u.id === state.currentUser?.id;
          const targetRank = ROLE_RANK[u.role] || 0;
          // Kasir hanya boleh mengubah akunnya sendiri (password saja).
          const canEdit = selfOnly ? isSelf : isFullAccess() ? true : targetRank < currentRank;
          ...
          return `
            <tr class="hover:bg-slate-50">
              <td class="py-2.5 px-3 font-bold text-slate-800">${u.full_name}${isSelf ? ' <span class="text-[9px] text-slate-400 font-normal">(Anda)</span>' : ''}</td>
                ...
                ${
                  canEdit
                    ? `<button onclick="window.openEditEmployeeModal('${u.id}')" class="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-[11px] font-bold text-slate-700">
                        <i class="fa-solid fa-key"></i> ${selfOnly ? 'Ubah Password' : 'Edit'}
                      </button>`
    ...
    tbody.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-rose-500">${
      err.message || 'Gagal memuat data akun.'
    }</td></tr>`;
  }
}
...
// Menampilkan/menyembunyikan field non-password di modal (dipakai untuk mode Kasir)
function setNonPasswordFieldsVisible(visible) {
  document.getElementById('employee-fullname-row')?.classList.toggle('hidden', !visible);
  document.getElementById('employee-username-row')?.classList.toggle('hidden', !visible);
  document.getElementById('employee-role-row')?.classList.toggle('hidden', !visible);
}

export function openAddEmployeeModal() {
  if (isCashier()) return; // Kasir tidak bisa menambah akun

  passwordOnlyMode = false;
  setNonPasswordFieldsVisible(true);
  ...
  document.getElementById('employee-fullname-input').required = true;
  document.getElementById('employee-username-input').value = '';
  document.getElementById('employee-username-input').required = true;
  ...
  populateRoleOptions();
  document.getElementById('employee-role-input').required = true;
...
export function openEditEmployeeModal(id) {
  ...
  const isSelf = u.id === state.currentUser?.id;
  passwordOnlyMode = isCashier();

  const passwordInput = document.getElementById('employee-password-input');

  if (passwordOnlyMode) {
    // Kasir: hanya boleh ganti password akunnya sendiri.
    if (!isSelf) return;

    setNonPasswordFieldsVisible(false);
    document.getElementById('employee-modal-title').innerText = 'Ubah Password Saya';
    document.getElementById('employee-id-input').value = u.id;
    document.getElementById('employee-fullname-input').required = false;
    document.getElementById('employee-username-input').required = false;
    document.getElementById('employee-role-input').required = false;
    passwordInput.value = '';
  ...
  setNonPasswordFieldsVisible(true);
  document.getElementById('employee-modal-title').innerText = isSelf ? 'Ubah Akun Saya' : 'Ubah Data Karyawan';
  document.getElementById('employee-id-input').value = u.id;
...
export async function submitEmployeeForm() {
  ...
  try {
    ...
    if (passwordOnlyMode) {
      ...
      result = await api.put(`/api/employees/${id}`, { password });
    } else {
    ...
    if (result.success) {
      showToast(result.message || 'Data akun berhasil disimpan.');
      closeEmployeeModal();
      loadEmployees();
    } else {
